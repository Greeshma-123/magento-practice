...
class CancelThread extends Column {
    /** Url path */
    const URL_PATH_RESPONSE = 'thread/chat/cancel';
    /** @var UrlBuilder */
    ...
    public function prepareDataSource(array $dataSource) {
        if (isset($dataSource['data']['items'])) {
            foreach ($dataSource['data']['items'] as & $item) {
                ...
                if (isset($item['id'])) {
                    $item[$name]['edit'] = ['href' => $this->urlBuilder->getUrl(self::URL_PATH_RESPONSE, ['id' => $item['id']]), 'label' => __('Cancel') ];
                }
            }
        }
        return $dataSource;
